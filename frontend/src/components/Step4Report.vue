                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  <div class="loading-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <circle cx="12" cy="12" r="10" stroke-width="4" stroke="var(--border-soft)"></circle>
                      <path d="M12 2a10 10 0 0 1 10 10" stroke-width="4" stroke="var(--text-300)" stroke-linecap="round"></path>
                    </svg>
                  </div>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          <div class="workflow-metrics">
            <div class="metric">
              <span class="metric-label">{{ $t('step4.metaSections') }}</span>
              <span class="metric-value mono">{{ completedSections }}/{{ totalSections }}</span>
            </div>
            <div class="metric">
              <span class="metric-label">{{ $t('step4.metaElapsed') }}</span>
              <span class="metric-value mono">{{ formatElapsedTime }}</span>
            </div>
            <div class="metric">
              <span class="metric-label">{{ $t('step4.metaTools') }}</span>
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'report_complete'">
                    <div class="complete-banner">
                      ...
                      <span>{{ $t('step4.generationComplete') }}</span>
                    </div>
                  </template>
                </div>
...
.section-number {
  ...
  font-size: 16px;
  color: var(--text-400); /* Dark gray, does not change with status */
  font-weight: 500;
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--bg-3); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--border-soft);
  --wf-divider: var(--bg-3);

  --wf-active-bg: var(--bg-2);
  --wf-active-border: var(--text-200);
  --wf-active-dot: var(--text-200);
  --wf-active-text: var(--text-200);

  --wf-done-bg: var(--bg-2);
  --wf-done-border: var(--border-soft);
  --wf-done-dot: var(--status-green);

  --wf-muted-dot: var(--border-strong);
  --wf-todo-text: var(--text-400);
}
