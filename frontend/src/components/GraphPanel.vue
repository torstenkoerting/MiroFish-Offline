    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graph.stillProcessingHint') }}</span>
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            ...
            <span v-if="selectedItem.type === 'node'" class="detail-type-badge" :style="{ background: selectedItem.color, color: 'var(--text-100)' }">
...
const entityTypes = computed(() => {
  ...
  const colors = ['var(--lime)', '#004E89', '#7B2D8E', 'var(--status-green)', '#C5283D', '#E9724C', '#3498db', '#9b59b6', '#27ae60', '#f39c12']
...
const renderGraph = () => {
  ...
  const getColor = (type) => colorMap[type] || 'var(--text-400)'
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', 'var(--text-400)').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', 'var(--text-300)')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      node.attr('stroke', 'var(--bg-2)').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', 'var(--text-400)').attr('stroke-width', 1.5)
    ...
    .on('mouseenter', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', 'var(--text-200)').attr('stroke-width', 3)
      }
    })
    .on('mouseleave', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', 'var(--bg-2)').attr('stroke-width', 2.5)
      }
    })
